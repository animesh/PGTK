...
def main():
 ...
 for r in sorted(ss,key=lambda x:(x['Subject'],x['Sample'])):lines.append(f"- {r['Sample']} ({r['Subject']}): {r['ProgressionAlleles']} alleles, {r['ProgressionGenes']} genes, {r['GO_TermsTested']} GO terms tested, {r['SignificantGOTerms']} significant at FDR <= {r['FDRThreshold']}")
