...
with tempfile.TemporaryDirectory() as d:
 ...
 subprocess.run([*PYTHON,str(R/'prepare_go_annotations.py'),'--obo',str(obo),'--gaf',str(gaf),'--output-prefix',str(w/'go')],check=True)
 ...
 for s,rows in {'P1':[(10,'GENE1'),(20,'GENE3')],'P2':[(10,'GENE1'),(30,'GENE2')]}.items():
  ...
  subprocess.run([*PYTHON,str(R/'analyze_progression_biology.py'),'--sample',s,'--subject','T1','--group',s,'--progression-vcf',str(pv),'--background-vcf',str(bv),'--go-mapping',str(w/'go.mapping.tsv'),'--go-min-size','1','--go-max-size','10','--fdr-threshold','0.1','--output-prefix',str(pre)],check=True);outs[s]=pre
  summary=rd(str(pre)+'.summary.tsv');assert summary[0]['FDRThreshold']=='0.1';assert 'SignificantGOTerms' in summary[0];assert 'SignificantGOTermsFDR05' not in summary[0]
 e=rd(str(outs['P1'])+'.go_enrichment.tsv');assert {'GO:0000001','GO:0000002','GO:0000003'}<=set(r['GO_ID'] for r in e)
 pp=w/'pair';subprocess.run([*PYTHON,str(R/'compare_progression_pair.py'),'--subject','T1','--sample-a','P1','--sample-b','P2','--alleles-a',str(outs['P1'])+'.alleles.tsv','--alleles-b',str(outs['P2'])+'.alleles.tsv','--genes-a',str(outs['P1'])+'.genes.tsv','--genes-b',str(outs['P2'])+'.genes.tsv','--go-a',str(outs['P1'])+'.go_enrichment.tsv','--go-b',str(outs['P2'])+'.go_enrichment.tsv','--output-prefix',str(pp)],check=True)
 assert rd(str(pp)+'.summary.tsv')[0]['SharedAlleles']=='1';assert len(rd(str(pp)+'.go_contrasts.tsv'))==2
 merged=w/'merged'
 subprocess.run([*PYTHON,str(R/'merge_progression_biology.py'),'--sample-alleles',str(outs['P1'])+'.alleles.tsv',str(outs['P2'])+'.alleles.tsv','--sample-genes',str(outs['P1'])+'.genes.tsv',str(outs['P2'])+'.genes.tsv','--sample-go',str(outs['P1'])+'.go_enrichment.tsv',str(outs['P2'])+'.go_enrichment.tsv','--sample-candidates',str(outs['P1'])+'.candidates.tsv',str(outs['P2'])+'.candidates.tsv','--sample-summary',str(outs['P1'])+'.summary.tsv',str(outs['P2'])+'.summary.tsv','--pair-alleles',str(pp)+'.alleles.tsv','--pair-genes',str(pp)+'.genes.tsv','--pair-go',str(pp)+'.go_contrasts.tsv','--pair-summary',str(pp)+'.summary.tsv','--go-metadata',str(w/'go.metadata.tsv'),'--output-prefix',str(merged)],check=True)
 report=Path(str(merged)+'.report.md').read_text()
 assert 'FDR <= 0.1' in report and 'FDR <= 0.05' not in report
 merged_summary=rd(str(merged)+'.multiqc_summary.tsv')
 assert all(r['FDRThreshold']=='0.1' and 'SignificantGOTerms' in r for r in merged_summary)
 print('PASS: versioned GO propagation, sample ORA, pair contrast and CLI-driven merged reporting')
